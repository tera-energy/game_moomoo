...
public class TrLobbyManager : MonoBehaviour
{
    ...
    [SerializeField] TrUI_HoldButton _btnFriend;
    ...
    public void xOnClickRandomMatching(){
        _anim.SetTrigger("Start");

        _trUIPuzzle.GetComponent<CanvasGroup>().DOFade(0, 1f);
        _imgFade.DOFade(1, 2f).OnComplete(() => SceneManager.LoadScene(TrProjectSettings.strMULTILOBBY));
    }
    ...
    IEnumerator yWaitCheckVersion()
    {
        ...
        if (_isFirstLobby)
        {
            ...
            yield return new WaitUntil(() => DatabaseManager._myDatas.nickName != "" && DatabaseManager._myDatas.nickName != null);
