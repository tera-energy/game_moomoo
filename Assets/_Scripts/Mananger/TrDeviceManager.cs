...
public class TrDeviceManager : MonoBehaviour
{
    ...
    IEnumerator yCheckNetwork(){
        if (!TrCheckInternet.zGetIsConnectNetwork())
            TrCheckInternet.xInstance.zActiveConnectWindow(true);
        yield return new WaitUntil(() => TrCheckInternet.zGetIsConnectNetwork());
        TrCheckInternet.xInstance.zActiveConnectWindow(false);
